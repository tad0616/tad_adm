    ...
    <tbody>
        <{foreach from=$all_un_theme item=mod}>
            <tr>
                ...
                <{if $mod.function=='install_theme'}>
                    <a href="main.php?op=install_theme&dirname=<{$mod.dirname}>&file_link=<{$mod.file_link}>&update_sn=<{$mod.update_sn}>&tad_adm_tpl=clean" class="btn btn-xs btn-primary modulesadmin"  data-fancybox-type="iframe"><{$smarty.const._MA_TADADM_MOD_INSTALL_THEME}></a>
                <{/if}>
                </td>
                <td nowrap style="text-align:center;">
                <{if $mod.descript}>
