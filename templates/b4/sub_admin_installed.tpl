    ...
    <div class="<{if $mod.function=='update_adm_tpl'}>alert alert-danger<{elseif $mod.function=='unable_adm_tpl'}>well<{else}>alert alert-success<{/if}>">
        <div class="row">
            <div class="col-sm-6 col-md-5 col-lg-4">
                <img src="../images/tad_adm_tpl.png" alt="<{$mod.name}>" id="<{$mod.dirname}>_tip" title="<{$smarty.const._MA_TADADM_ADMTPL}>" class="img-fluid">
            </div>
            <div class="col-sm-6 col-md-7 col-lg-8">
                ...
                <{if $mod.function=='update_adm_tpl'}>
                    <a href="main.php?op=update_adm_tpl&dirname=<{$mod.dirname}>&file_link=<{$mod.file_link}>&update_sn=<{$mod.update_sn}>&tad_adm_tpl=clean" class="btn btn-sm btn-danger modulesadmin" data-fancybox-type="iframe">
                        <{$smarty.const._MA_TADADM_MOD_UPDATE_ADMTPL}>
                            <{$mod.new_version}>
                    </a>
                <{elseif $mod.function=='unable_adm_tpl'}>
                    <a href="#" class="btn btn-sm modulesadmin" disabled style="margin:6px 0px;">
                        <{$mod.status}>
                    </a>
                <{else}>
