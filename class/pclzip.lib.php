...
//    $v_path = "./";
      ...
      public function privOptionDefaultThreshold(&$p_options)
      {
        ...
        || isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF])) {
          ...
          $last = mb_strtolower(mb_substr($v_memory_limit, -1));
      ...
      public function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
      {
          ...
          // ----- Loop on the files
          for ($j = 0; ($j < count($p_filedescr_list)) && (1 == $v_result); $j++) {
              // ----- Format the filename
      ...
      public function privAddFile($p_filedescr, &$p_header, &$p_options)
      {
          ...
          */
          ...
          elseif ('virtual_file' === $p_filedescr['type']) {
              $p_header['external'] = 0x00000000;
              $p_header['size'] = mb_strlen($p_filedescr['content']);
          }
          ...
          if (isset($p_filedescr['comment'])) {
              $p_header['comment_len'] = mb_strlen($p_filedescr['comment']);
                ...
                || (isset($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD])
                  ...
                  // ----- Use "in memory" zip algo
                      ...
                      // ----- Look for normal compression
                          ...
                          // ----- Set header parameters
                          $p_header['compressed_size'] = mb_strlen($v_content);
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_ADD].'(PCLZIP_CB_POST_ADD, $v_local_header);');
      ...
      public function privWriteFileHeader(&$p_header)
      {
          ...
          if (0 != mb_strlen($p_header['stored_filename'])) {
              fwrite($this->zip_fd, $p_header['stored_filename'], mb_strlen($p_header['stored_filename']));
          }
      ...
      public function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
      {
          ...
          if (0 != mb_strlen($p_comment)) {
              fwrite($this->zip_fd, $p_comment, mb_strlen($p_comment));
          }
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_EXTRACT].'(PCLZIP_CB_POST_EXTRACT, $v_local_header);');
      ...
      public function privReadFileHeader(&$p_header)
      {
          ...
          if (26 != mb_strlen($v_binary_data)) {
              ...
              // ----- Error log
              self::privErrorLog(PCLZIP_ERR_BAD_FORMAT, 'Invalid block size : ' . mb_strlen($v_binary_data));
      ...
      public function privReadEndCentralDir(&$p_central_dir)
      {
          ...
          if (18 != mb_strlen($v_binary_data)) {
              // ----- Error log
              self::privErrorLog(PCLZIP_ERR_BAD_FORMAT, 'Invalid End of Central Dir Record size : ' . mb_strlen($v_binary_data));
      ...
      public function privDeleteByRule(&$p_result_list, &$p_options)
      {
          ...
          if ($v_nb_extracted > 0) {
              ...
              for ($i = 0; $i < count($v_header_list); $i++) {
                  ...
                  if (1 != $this->privCheckFileHeaders(
                      $v_local_header,
                      $v_header_list[$i]
            )) {
                      // TBC
              ...
              // ----- Create the central dir footer
              if (1 != ($v_result = $v_temp_zip->privWriteCentralHeader(count($v_header_list), $v_size, $v_offset, $v_comment))) {
                  // ----- Reset the file list
                  unset($v_header_list);
                  $v_temp_zip->privCloseFd();
                  $this->privCloseFd();
