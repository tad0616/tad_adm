    ...
    public function addLineConfigLabel($text, $value = '', $type = 'default')
    {
        ...
        switch ($type) {
            ...
            case 'php':
                ...
                break;
            case 'xoops':
                if (mb_substr(XOOPS_VERSION, 0, 9) < $value) {
                    $line .= "<span style='color : red; font-weight : bold;'><img src='" . $path . "off.png' >" . sprintf($text, $value, mb_substr(XOOPS_VERSION, 0, 9)) . "</span>\n";
                } else {
                    $line .= "<span style='color : green;'><img src='" . $path . "on.png' >" . sprintf($text, $value, mb_substr(XOOPS_VERSION, 0, 9)) . "</span>\n";
                }
                break;
            case 'folder':
                if (!is_dir($value)) {
            ...
            case 'chmod':
                if (is_dir($value[0])) {
                    if (mb_substr(decoct(fileperms($value[0])), 2) != $value[1]) {
                        $line .= "<span style='color : red; font-weight : bold;'><img src='" . $path . "off.png' >" . sprintf($text, $value[0], $value[1], mb_substr(decoct(fileperms($value[0])), 2)) . "</span>\n";
                    } else {
                        $line .= "<span style='color : green;'><img src='" . $path . "on.png' >" . sprintf($text, $value[0], $value[1], mb_substr(decoct(fileperms($value[0])), 2)) . "</span>\n";
                    }
                }
                break;
        }
